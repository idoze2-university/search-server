...
void ParallelServer::open(int port, IClientHandler *c)
{
    ...
    isOpen = true;
    auto server_listen_thread = thread(&ParallelServer::serverListen, this, &address, socketfd, c);
...
void ParallelServer::serverListen(sockaddr_in *address, int socketfd, IClientHandler *handler)
{
    try
    {
        while (isOpen)
